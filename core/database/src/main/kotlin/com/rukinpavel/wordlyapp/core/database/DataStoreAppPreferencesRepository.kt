...
class DataStoreAppPreferencesRepository @Inject constructor(
...
) : AppPreferencesRepository, UserPreferencesRepository {
    ...
    private object PreferencesKeys {
        ...
        // Temporarily keeping these for shared implementation
        val HINT_COUNT = intPreferencesKey("hint_count")
    }

    override val language: Flow<Language?> = context.dataStore.data.map { preferences ->
    ...
    override val tutorialCompleted: Flow<Boolean> = context.dataStore.data.map { preferences ->
        preferences[PreferencesKeys.TUTORIAL_COMPLETED] ?: false
    }

    override val isPremium: Flow<Boolean> = context.dataStore.data.map { preferences ->
    ...
    // These will be moved to feature-specific implementations later
    ...
    override val hintCount: Flow<Int> = context.dataStore.data.map { preferences ->
